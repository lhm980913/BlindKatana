...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		protected virtual void StickToSlope()
        {
            ...
            _stickRaycastLeft = MMDebug.RayCast(_rayCastOrigin, -transform.up, rayLength, PlatformMask, MMColors.LightBlue, Parameters.DrawRaycastsGizmos);

            _rayCastOrigin.x = _boundsBottomRightCorner.x;
            _rayCastOrigin.x += _newPosition.x;
            _stickRaycastRight = MMDebug.RayCast(_rayCastOrigin, -transform.up, rayLength, PlatformMask, MMColors.LightBlue, Parameters.DrawRaycastsGizmos);
            ...
            if ((belowSlopeAngleLeft > 0f) && (belowSlopeAngleRight < 0f))
            {
                _stickRaycast = MMDebug.BoxCast(_boundsCenter, Bounds, Vector2.Angle(transform.up, Vector2.up), -transform.up, rayLength, PlatformMask, Color.red, true);
                if (_stickRaycast)
                {
        ...
        protected virtual void ComputeDistanceToTheGround()
        {
            ...
            _distanceToTheGroundRaycast = MMDebug.RayCast(_rayCastOrigin, -transform.up, DistanceToTheGroundRayMaximumLength, PlatformMask, MMColors.CadetBlue, false);
